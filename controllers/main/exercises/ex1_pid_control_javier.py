...
# from ex0_rotations import quaternion2rotmat

class quadrotor_controller():
    def __init__(self):
        # Exercise 1: Choose what to tune ["vel_z", "pos_z", "vel_xy", "pos_xy"]  
        self.tuning_level = "pos_z"
        
        # Only change the gains you are asked to, the others are already tuned by us   ### as seen in the lectures: it takes a position and a yaw setpoint as input and then generates pwm signals for the motors as outputs.
        gains = {
    ...
    def setpoint_to_pwm(self, dt, setpoint, sensor_data):
        ...
        yaw_setpoint = setpoint[3] #0 yaw? 

        # Calculate rotation

        #need to generate. 
        #since quaternions are optimal, I will use the quaternion data from main file
        # sensor_data['q_x'] = self.imu.getQuaternion()[0]
        # sensor_data['q_y'] = self.imu.getQuaternion()[1]
        # sensor_data['q_z'] = self.imu.getQuaternion()[2]
        # sensor_data['q_w'] = self.imu.getQuaternion()[3]

        quaternion = [sensor_data['q_x'], sensor_data['q_y'], sensor_data['q_z'], sensor_data['q_w']]
        R_inertial_to_body = R.from_quat(quaternion).as_matrix().T


        # Rotate position error into body frame
        pos_error_body = R_inertial_to_body @ np.array([pos_x_error, pos_y_error, pos_z_error])

        # Put setpoint of PID controller
        self.pid_pos_x.setpoint = pos_error_body[0]
        self.pid_pos_y.setpoint = pos_error_body[1]
        self.pid_pos_z.setpoint = pos_error_body[2]
        ...
        vel_error_x = vel_x_setpoint - sensor_data["v_forward"]
        vel_error_y = vel_y_setpoint - sensor_data["v_left"]
        vel_error_z = vel_z_setpoint - sensor_data["v_up"]

        # Put setpoint of PID controller
        self.pid_vel_x.setpoint = vel_error_x #vel.xsetpoint
        self.pid_vel_y.setpoint = vel_error_y
        self.pid_vel_z.setpoint = vel_error_z

        # Call PID controller (use sensor_data["v_forward"], sensor_data["v_left"], sensor_data["v_up"])
        acc_x_setpoint = self.pid_vel_x(measurement, dt=dt) #sensor_data
